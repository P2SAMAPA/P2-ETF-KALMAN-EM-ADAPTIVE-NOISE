...
st.set_page_config(page_title="Adaptive Kalman Filter with EM (Shumway & Stoffer)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🔄 Adaptive Kalman Filter with EM</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Shumway & Stoffer – Online EM estimation of noise covariances Q, R</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Kalman-EM")
...
st.sidebar.markdown(f"**EM iterations:** {config.EM_ITERATIONS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'kalman_em_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Kalman score: {etf['kalman_score_norm']:.3f}</p>
...
with tab1:
    st.header("🔄 Top ETFs by Adaptive Kalman Filter Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Adaptive Kalman filter with EM** (Shumway & Stoffer) estimates process noise Q and measurement noise R online.
        - The model assumes a random walk latent state: x_t = x_{t-1} + w, and observation y_t = x_t + v.
        - EM iterations re-estimate Q and R after each smoothing pass.
        - The final filtered state estimate (x_T|T) serves as the score – a denoised trend signal.
        - Higher score suggests upward momentum after accounting for adaptive noise levels.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the adaptive Kalman scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Adaptive Kalman Filter with EM | Shumway & Stoffer")
