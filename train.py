...
from kalman_em import kalman_em_score
...
def run_for_window(returns, window_days):
    ...
    for ticker in ret_window.columns:
        score = kalman_em_score(ret_window[ticker], em_iter=config.EM_ITERATIONS)
        raw_scores[ticker] = score
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "kalman_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/kalman_em_{timestamp}.json"
